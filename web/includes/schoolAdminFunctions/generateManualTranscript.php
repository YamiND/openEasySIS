...
{
   	$_SESSION['fail'] = 'Manual Transcript Generation failed, invalid permissions';
   	header('Location: ../../pages/generateManualTranscript');
}
...
function parseCSV($mysqli)
{
	...
	{
	   	$_SESSION['fail'] = 'Manual Transcript Generation failed, invalid CSV';
   		header('Location: ../../pages/generateManualTranscript');
	} 
	else 
	{
		if($_FILES['csvFile']['error'] == 0)
		{
   		 	...
   		 	if($ext === 'csv')
			{
				...
				{
	   				$_SESSION['fail'] = 'Manual Transcript Generation failed, data not sent';
			   		header('Location: ../../pages/generateManualTranscript');
				}
			}
			else
			{
	   			$_SESSION['fail'] = 'Manual Transcript Generation failed, file not a CSV';
			   	header('Location: ../../pages/generateManualTranscript');
		...
		{
			$_SESSION['fail'] = 'Manual Transcript Generation Failed, CSV upload issue';
	   		header('Location: ../../pages/generateManualTranscript');
		}
	}
}
...
$_SESSION['success'] = "Manual Transcript Generated";
//Close and output PDF document to the browser
$pdf->Output("$studentName-manualTranscript.pdf", 'D');
