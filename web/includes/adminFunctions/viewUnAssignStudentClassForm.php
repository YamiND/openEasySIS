...
echo '
                                ...
                                {
                                    unAssignStudentForm($_SESSION['classID'], $_SESSION['gradeID'], $mysqli);
                                }
echo '
                                </div>
                                <br>
...
function unAssignStudentForm($classID, $gradeID, $mysqli)
{
    if ($stmt = $mysqli->prepare("SELECT userID FROM users WHERE userID IN (SELECT userID from users WHERE studentGradeLevel = ?) AND userID IN (SELECT studentID FROM studentClassIDs WHERE classID LIKE ?)"))
    {
